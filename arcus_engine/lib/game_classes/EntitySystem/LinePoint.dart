...
class LinePoint {
  ...
  setToPolar(azimuth, radius, asDegrees) {
    ...
    if (asDegrees) {
      azimuth = Utils.shared.degreesToRadians(azimuth);
    }
  ...
  /**
     ...
     */
  ceil() {
    return this.setTo((this.x).ceil(), (this.y).ceil());
  }
 ...
 */
  negative(a) {
    LinePoint out = LinePoint(a.x, a.y);
 ...
 */
  rotatePoint(a, x, y, angle, asDegrees, distance) {
    if (asDegrees) {
      angle = Utils.shared.degreesToRadians(angle);
    }
 ...
 * @return {Phaser.Point} The new Point object.
 */
  centroid(List<LinePoint> points) {
    LinePoint out = LinePoint(0, 0);

    int pointslength = points.length;

    if (pointslength < 1) {
      print('Phaser.Point. Parameter points array must not be empty');
    }
 ...
 */
  trunc(obj) {
    obj.x = Utils.shared.trunc(obj.x);
    obj.y = Utils.shared.trunc(obj.y);
 ...
 */
  isPoint(obj) {
    return (obj != null) && (obj.x is double) && (obj.y is double);
  }
 ...
 * @return {Phaser.Point} The new Point object.
 */
  parse(obj, xProp, yProp) {
    var point = LinePoint(0, 0);
