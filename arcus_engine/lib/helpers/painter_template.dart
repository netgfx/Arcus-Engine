...
class Shadows extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawCircle(
        this.light.x.toDouble(), this.light.y.toDouble(), this.wallPaint!);
  ...
  void performRayCasting() {
    ...
    for (var i = 0; i < this.walls.length; i++) {
      ...
      for (var i = 0; i < corners.length; i++) {
        ...
        if (c.x == this.light.x) {
          ...
          var left = Point(0, b);
          var right = Point(
              this.sceneSize.maxWidth, slope * this.sceneSize.maxWidth + b);
          var top = Point(-b / slope, 0);
          var bottom = Point(
              (this.sceneSize.maxHeight - b) / slope, this.sceneSize.maxHeight);
        ...
        // Create a ray
        ray = Line(Point(this.light.x.toDouble(), this.light.y.toDouble()),
            Point(end!.x.toDouble(), end.y.toDouble()));
    ...
    for (var i = 0; i < stageCorners.length; i++) {
      ray = Line(Point(this.light.x, this.light.y),
          Point(stageCorners[i].x, stageCorners[i].y));
      intersect = this.getWallIntersection(ray);
      //delayedPrint(intersect.toString());
      if (intersect == null) {
        // Corner is in light
    ...
    points.sort((a, b) {
      ...
      var det = (a.x - center.x) * (b.y - center.y) -
      ...
      var d1 = (a.x - center.x) * (a.x - center.x) +
          (a.y - center.y) * (a.y - center.y);
      var d2 = (b.x - center.x) * (b.x - center.x) +
  ...
  Point? getWallIntersection(Line ray) {
    ...
    this.walls.forEach((wall) {
      ...
      List<Line> lines = [
        ...
        Line(Point(wall.x + wallWidth, wall.y),
            Point(wall.x + wallWidth, wall.y + wallHeight)),
        Line(Point(wall.x, wall.y + wallHeight),
            Point(wall.x + wallWidth, wall.y + wallHeight))
      ];
      ...
      for (var i = 0; i < lines.length; i++) {
        var intersect = intersectsPoints(ray.getStart(), ray.getEnd(),
            lines[i].getStart(), lines[i].getEnd());
        // delayedPrint(intersect.toString());
        if (intersect != null) {
          // Find the closest intersection
          var distance =
              this.distance(ray.start.x, ray.start.y, intersect.x, intersect.y);
  ...
  void drawWalls() {
    for (var i = 0; i < this.walls.length; i++) {
      drawRect(this.walls[i].x.toDouble(), this.walls[i].y.toDouble(), 40, 40,
          this.wallPaint!);
    }
  }
  ...
  void drawCones(List<Point> points) {
    ...
    for (var k = 0; k < points.length; k++) {
      canvas!.drawLine(Offset(this.light.x.toDouble(), this.light.y.toDouble()),
          Offset(points[k].x - 2, points[k].y - 2), this.paintStroke!);
    }
  }
  ...
  void drawRect(
      double x, double y, double? width, double? height, Paint paint) {
    if (this.canvas != null) {
      rotate(x, y, () {
  ...
  void drawPolygon(double x, double y, int num, Paint paint,
      {double initialAngle = 0}) {
    rotate(x, y, () {
      final Path path = Path();
      for (int i = 0; i < num; i++) {
        final double radian =
            vectorMath.radians(initialAngle + 360 / num * i.toDouble());
