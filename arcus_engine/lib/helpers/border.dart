  ...
  BorderCanvas({
    ...
    for (var i = 0; i < totalDashesX; i++) {
      ...
      var point = {
        ...
        "targetPoint": Point(finalPosX, posY)
      };
      side1.add(point);
      posX = finalPosX;
    }
    ...
    for (var i = 0; i < totalDashesY; i++) {
      ...
      var point = {
        "point": Point(posX, posY),
        "targetPoint": Point(posX, finalPosY)
      };
      side2.add(point);
      posY = finalPosY;
    }
    ...
    for (var i = 0; i < totalDashesX; i++) {
      ...
      var finalPosX = posX + dashWidth;
      var point = {
        "point": Point(finalPosX, posY),
        "targetPoint": Point(posX, posY)
      };
      side3.add(point);
    }
    posX = 0;
    ...
    if (this.delay > 0 && this.animate != null) {
      Future.delayed(
          Duration(milliseconds: this.delay), () => {this.animate!()});
    }
  }
    ...
    if (this.controller != null) {
      if (this.controller!.lastElapsedDuration != null) {
        ...
        if (this.controller!.lastElapsedDuration!.inMilliseconds -
          ...
          int elapsed = (this.controller!.lastElapsedDuration!.inMilliseconds -
              this.currentTime);
          this.currentTime =
          ...
          for (var i = 0; i < points.length; i++) {
            ...
            var x =
                ui.lerpDouble(points[i]["point"].x, nextPos["point"].x, endT)!;
            var y =
                ui.lerpDouble(points[i]["point"].y, nextPos["point"].y, endT)!;

            var targetX = ui.lerpDouble(
                points[i]["targetPoint"].x, nextPos["targetPoint"].x, endT)!;
            var targetY = ui.lerpDouble(
                points[i]["targetPoint"].y, nextPos["targetPoint"].y, endT)!;
